...
At a first glance, the majority of the individuals are single, with or without children (lighter colors) after separation. With time passing, some individuals experience unions (darker colors) and at the end of the period about half of the individuals are still single.
...
We then labelled the clusters by looking at the full sequence index plot by cluster. In order to avoid overplotting, we sorted the sequences according to their distance to the most representative sequence, defined by the neighborhood density.
...
cl.6fac <- factor(wardrange.OM.6, labels = paste("Type", 1:6))
...
* Cluster 1 - Single parents (shorter sequences): individuals who had a child b/ separation, remain single for 2-3 years after separation and then are not followed anymore
...
* Cluster 3 - Parents ever single: individuals who had a child b/ separation and remain single over the whole period

* Cluster 4: Childless single entering in a union and having a child
...
* Cluster 6 - Single becoming parents: childless single or single who already had a child b/ separation having a child approximately one year after separation
...
```
By chosing again the solution with 6 clusters, also the average silhouette improve ($0.72$).
...
OM.T.distRepseq <- seqdist(seqObj2, refseq = repseq2, method = "OM", 
                        indel=1, sm="TRATE") 
# Define the cluster variable
cl.6fac2 <- factor(wardrange.OM.T6, labels = paste("Type", 1:6))
# Plot the full sequence index plot
