...
periods <- 25
n.a <- matrix(NA,nrow=dim(U.pen1)[1],ncol=periods+1)
n.b <- matrix(NA,nrow=dim(U.pen1)[1],ncol=periods+1)
n.c <- matrix(NA,nrow=dim(U.pen1)[1],ncol=periods+1)
...
We created a list of 117 elements collecting annual $\mathbf{A}$ matrices. Each $\mathbf{A}$ matricx has fertility in the first row, survival probabilities on the subdiagonal.
...
for(i in 1:nby) {
  ...
  ## Extract eigen-values (lambdas)
  A.eigen[,i] <- eigen(A.y[[i]])$values
  
  ## Population growth rate (dominant eigenvalue, lambda1)
...
We plotted the $\lambda$ values over time.

```{r, fig.weight=2, fig.height=4}
years <- c(1891:2007)
db_lambda <- data.frame(Year=years, Lambda=as.numeric(lambda1))
